...
export async function createVapidJwt(
  ...
  if (subject.includes('@') && !subject.startsWith('mailto:') && !subject.startsWith('https://')) {
...
export async function encryptPayload(
  ...
  const ephemeralPublicKey = await crypto.subtle.exportKey('raw', ecdhPair.publicKey);
  ...
  const webPushInfo = new Uint8Array(
    infoPrefix.length + recipientPublicBytes.length + ephemeralPublicBytes.length
  );
  webPushInfo.set(infoPrefix, 0);
  webPushInfo.set(recipientPublicBytes, infoPrefix.length);
  webPushInfo.set(ephemeralPublicBytes, infoPrefix.length + recipientPublicBytes.length);
  ...
  result.set(idlen, offset); offset += 1;
  result.set(ephemeralPublicBytes, offset); offset += ephemeralPublicBytes.length;
